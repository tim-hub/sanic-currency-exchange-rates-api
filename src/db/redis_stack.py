...
class RedisJSONRepo(AbstractDBInstace):
    db = None
    jsonDB = None

    @staticmethod
    def self_init():
        RedisJSONRepo.db = redis.from_url(getenv('REDIS_URL', 'redis://localhost:6379'))
        RedisJSONRepo.jsonDB = RedisJSONRepo.db.json()
    ...
    def get_single_rate(dt):
        ...
        rates = RedisJSONRepo.jsonDB.get(date, '$')
    ...
    async def get_historic_rates(start_at, end_at):
        ...
        while the_date <= end_at:
            day_rates = RedisJSONRepo.get_single_rate(the_date)
            if day_rates:
                results.append({
                    'date': the_date.date(),
                    'rates': day_rates
    ...
    async def get_rates(dt):
        checking_date = dt
        rates = RedisJSONRepo.get_single_rate(checking_date)
        while not rates:
    ...
    async def upsert_rates_by_time(ratesData):
        ...
        rates = RedisJSONRepo.jsonDB.get(ratesData.attrib["time"])

        if not rates:
            RedisJSONRepo.jsonDB.set(
